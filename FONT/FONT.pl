...
my $dirIn = 'c:\games\QUIKMENU';
my $dirOut = 'c:\games\QB';
my $lookUp = 0;
my $verbose = 2; # 0/1/2
my $test = 1;
...
Print ("Fonts: @fonts") if $verbose;

foreach (@fonts) {
    Print ("$_");
    ConvertFile("$dirIn\\$_.CHR", "$dirOut\\$_.DAT");
}
...
sub ProcessOperation {
    ...
    Print ("%s %d, %d", $op ? ($op == 1 ? "DRAW" : "END") : "MOVE", $x, $y) if $verbose == 2;
...
sub ProcessVectorData {
    ...
    Print ("Addr: $offset, $remaining") if $verbose;
...
sub ConvertFile {
    ...
    if ($firstChar > 0x20 || $lastChar < 0x7F) {
        Error ("%s doesn't contain the chars we need", $input);
    } elsif ($lastChar > 0x100) {
...
source: https://github.com/cc65/cc65/blob/master/src/chrcvt65/main.c
